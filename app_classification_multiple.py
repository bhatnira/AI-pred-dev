...
# Added components import to safely embed HTML content like LIME outputs
...
"""
        ...
        roc_auc_msg = f'Multi-class discrimination ability ({roc_auc:.4f})' if roc_auc is not None else 'Not available'
...
def predict_from_single_smiles_multiclass(smiles, featurizer_name, model, label_encoder, class_names=None, X_train=None, featurizer_obj=None):
    ...
    try:
        ...
        display_classes = class_names if class_names is not None else label_encoder.classes__
                        ...
                        try:
                            ...
                            try:
                                ...
                                model_params = {
                                    ...
                                    'Test Size': f"{test_size:.2%}" if isinstance(test_size, (int, float)) else str(test_size),
                                    'Featurizer': st.session_state.selected_featurizer_name_multiclass,
                    ...
                    if explanation_html:
                        with st.expander("🔎 LIME Explanation", expanded=False):
                            try:
                                ...
                                st.info("LIME explanation available as download.")
                            safe_smiles = "".join(c if c.isalnum() or c in "-_." else "_" for c in smile_input) if isinstance(smile_input, str) else "molecule"
                            st.download_button(
                                label="Download LIME HTML",
                                data=explanation_html.encode("utf-8"),
                                file_name=f"lime_explanation_{safe_smiles}.html",
                                mime="text/html"
                            )
                else:
