...
def render_navigation_bar():
    ...
    if 'reg_active_tab' not in st.session_state:
        st.session_state.reg_active_tab = "home"
    ...
    for idx, (label, key) in enumerate(nav_options.items()):
        with cols[idx]:
            is_active = st.session_state.reg_active_tab == key
            ...
            if st.button(
                ...
                if st.session_state.reg_active_tab != key:
                    st.session_state.reg_active_tab = key
    
    st.markdown("</div>", unsafe_allow_html=True)
    return st.session_state.reg_active_tab
